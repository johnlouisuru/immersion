            ...
            <div class="btn-group">
                 <button type="button" class="btn btn-sm btn-primary dropdown-toggle" data-bs-toggle="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><a class="dropdown-item" href="?section_id=all">All Sections</a></li>
                        <?php
                        //$section_ids = array_unique(array_column($students, 'section_id'));
                        foreach ($sections_dropdown as $sd):
                            //$section_info = get_section_name($pdo, $sd['id']);
                        ?>
                            <li>
                                <a class="dropdown-item" href="?section_id=<?= $sd["section_id"] ?>">
                                    <?= htmlspecialchars($sd['section_name']) ?>
                                </a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
...
document.querySelectorAll('.section-filter').forEach(btn => {
  btn.addEventListener('click', function(e) {
    e.preventDefault();
    const sectionId = (this.dataset.section || '').trim();

    document.querySelectorAll('#datatablesSimple tbody tr').forEach(row => {
      const rowSection = (row.dataset.section || '').trim();
      console.log("Filter:", sectionId, "Row:", rowSection);

      if (sectionId === 'all' || rowSection === sectionId) {
        row.style.display = '';
      } else {
